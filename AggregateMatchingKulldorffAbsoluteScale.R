dataPrep <- function(){
  ...
  secondTest <- left_join(testset, popZCTA, by = c("patzip" = "zipnum"))
  ...
  Xformatch <- dataMatch[,c("patzip", "month", "weekend","heatwave", "aggcode", "latitude.x", "longitude.x", "Total.population")]
...
col_num = 12 
diffByZipHeatWave<- aggregate(matchedDataset[,col_num], list(XHeat$patzip), mean)
